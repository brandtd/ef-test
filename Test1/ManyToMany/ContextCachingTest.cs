...
{
    public static class ContextCachingTest
    {
        ...
        public class RenameWithoutChangingContext
        {
            ...
            public void PrintIt()
            {
                Console.WriteLine($"Name under context {_db.GetHashCode()}: {_db.Students.Where(s => s.StudentId == 2).First().Name}");
            }

            public void SetIt(string name)
            {
                Console.WriteLine($"Changing name under context {_db.GetHashCode()}");
                _db.Students.Where(s => s.StudentId == 2).First().Name = name;
                _db.SaveChanges();
            }

            private ManyToManyDatabase _db;
        }
    }
}